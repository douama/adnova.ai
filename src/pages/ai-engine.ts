...
export function renderAIEngine(lang: Lang = 'en'): string {
  ...
  <div class="glass rounded-2xl p-6 mb-6 border border-brand-500/20 relative overflow-hidden" id="ai-hero-card">
    <div class="absolute inset-0 bg-gradient-to-r from-brand-950/50 to-brand-950/50"></div>
    <div class="relative flex items-center justify-between flex-wrap gap-4">
      <div class="flex items-center gap-4">
        <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center shadow-2xl relative" id="ai-brain-icon">
          <i class="fas fa-brain text-white text-2xl" id="ai-brain-icon-i"></i>
          <span class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-brand-500 border-2 border-slate-950 animate-pulse" id="ai-status-dot"></span>
        </div>
        <div>
          ...
          <div class="flex items-center gap-4 mt-2 text-xs text-slate-500 flex-wrap">
            ...
            <span><i class="fas fa-dollar-sign mr-1 text-brand-400"></i>$847 ROI generated today</span>
          </div>
        </div>
      </div>
      <div class="flex gap-3">
        ...
        <button onclick="openConfigModal()" class="bg-gradient-to-r from-brand-600 to-brand-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-all hover:opacity-90 flex items-center gap-2">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6" id="ai-modules-grid">
    ${aiModule('perf', 'Performance Predictor', 'fa-chart-line', 'from-brand-500 to-brand-600', 'active', '94.2%', 'Prediction accuracy', 'Uses historical ROAS, CTR, and audience data to predict campaign performance before launch. Identifies high-potential placements.', [
      ['Predictions today', '284'],
      ['Avg confidence', '87.4%'],
      ['Model version', 'GPT-4o fine-tuned']
    ])}
    ${aiModule('gen', 'Creative Generator', 'fa-wand-magic-sparkles', 'from-brand-500 to-brand-600', 'active', '12,840+', 'Creatives made', 'Generates images, videos, and UGC-style content using your brand brief. Automatically A/B tests and promotes winners.', [
      ['Generated today', '47'],
      ['Win rate', '34%'],
      ['Top format', 'UGC Video']
    ])}
    ${aiModule('budget', 'Budget Optimizer', 'fa-dollar-sign', 'from-brand-500 to-brand-600', 'active', '$124K', 'Managed today', 'Dynamically reallocates budget across platforms and campaigns based on real-time ROAS signals. Scales winners +10% every 72h.', [
      ['Reallocations today', '18'],
      ['Budget saved', '$2,340'],
      ['Avg ROAS lift', '+0.8x']
    ])}
    ${aiModule('aud', 'Audience Intelligence', 'fa-users', 'from-slate-500 to-slate-600', 'active', '47', 'Audiences managed', 'Builds predictive lookalike audiences from your best converters. Automatically expands reach when saturation is detected.', [
      ['New audiences built', '3'],
      ['Avg lookalike size', '2.1M'],
      ['Match rate', '84.2%']
    ])}
    ${aiModule('kill', 'Creative Killer', 'fa-scissors', 'from-slate-500 to-brand-600', 'active', '37', 'Ads paused today', 'Continuously monitors creative performance. Pauses ads below CTR threshold and reallocates budget to top performers.', [
      ['Kill threshold', 'CTR < 0.8%'],
      ['Budget recovered', '$4,200'],
      ['Avg time to kill', '4.2h']
    ])}
    ${aiModule('copy', 'Copy Engine', 'fa-pen-nib', 'from-brand-500 to-orange-600', 'active', '1,284', 'Headlines written', 'Generates and tests ad headlines, descriptions, and CTAs. Learns which copy converts best for each audience segment.', [
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
    <div class="glass rounded-2xl p-5">
      ...
      <div class="space-y-2 max-h-80 overflow-y-auto" id="ai-log">
        ${aiLogItem('arrow-trend-up', 'brand', 'Scale', '"Summer Collection" budget +10% — ROAS 5.26x ≥ 5.0x target', '2m ago')}
        ${aiLogItem('scissors', 'slate', 'Kill', 'Paused creative "old-banner-v1" — CTR 0.31% < 0.8% threshold', '8m ago')}
        ${aiLogItem('wand-magic-sparkles', 'brand', 'Create', 'Generated 3 UGC video variants for TikTok campaign', '15m ago')}
        ${aiLogItem('users', 'slate', 'Audience', 'Built 3% lookalike from top 1K converters — 2.4M reach', '22m ago')}
        ${aiLogItem('dollar-sign', 'brand', 'Budget', 'Moved $1.2K from Snapchat to TikTok — +18% predicted ROAS', '31m ago')}
        ${aiLogItem('chart-line', 'slate', 'Test', 'A/B test concluded: Video B wins — 34% better CTR, scaling', '45m ago')}
        ${aiLogItem('brain', 'brand', 'Learn', 'Model updated with 2,840 new conversion signals', '1h ago')}
        ${aiLogItem('arrow-trend-up', 'brand', 'Scale', '"Product Launch" budget +10% — criteria met', '2h ago')}
      </div>
    </div>
    ...
    <div class="glass rounded-2xl p-5">
      ...
      <div class="space-y-2" id="ai-config-list">
        ...
        ${aiConfigRow('kill-thresh', 'Kill Threshold', 'CTR < 0.8% after 500 impressions', 'slate')}
        ${aiConfigRow('scale-amt', 'Scale Amount', '+10% every 72 hours', 'brand')}
        ${aiConfigRow('max-budget', 'Max Budget Cap', '$5,000/day per campaign', 'brand')}
        ${aiConfigRow('creative-test', 'Creative Test Duration', 'Min 48h or 1,000 impressions', 'slate')}
        ${aiConfigRow('aud-expand', 'Audience Expand Trigger', 'Frequency > 3.5 or CPM +30%', 'brand')}
        ${aiConfigRow('budget-realloc', 'Budget Reallocation', 'Every 6 hours based on ROAS', 'slate')}
        ${aiConfigRow('model', 'AI Model', 'AdNova v2 + GPT-4o fine-tuned', 'brand')}
      </div>
      <button onclick="openEditRulesModal()" class="w-full mt-4 bg-gradient-to-r from-brand-600 to-brand-600 text-white py-2.5 rounded-xl text-sm font-semibold transition-all hover:opacity-90 flex items-center justify-center gap-2">
  ...
  <div class="glass rounded-2xl p-5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
      ${modelMetric('94.2%', 'ROAS Prediction Accuracy', 'brand')}
      ${modelMetric('87.6%', 'CTR Prediction Accuracy', 'slate')}
      ${modelMetric('91.3%', 'Budget Optimization Score', 'brand')}
      ${modelMetric('2.8M', 'Training Data Points', 'brand')}
    </div>
    <div style="position:relative;height:180px"><canvas id="aiPerfChart"></canvas></div>
  </div>
    ...
    <div class="glass rounded-2xl w-full max-w-lg animate-fadeIn" style="border:1px solid rgba(99,102,241,0.4)">
      ...
      <div class="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          <button onclick="saveConfig()" class="bg-gradient-to-r from-brand-600 to-brand-600 text-white px-6 py-2 rounded-xl text-sm font-semibold">Save Configuration</button>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="glass rounded-2xl w-full max-w-xl animate-fadeIn" style="border:1px solid rgba(99,102,241,0.4)">
      ...
      <div class="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          <button onclick="saveEditedRules()" class="bg-gradient-to-r from-brand-600 to-brand-600 text-white px-6 py-2 rounded-xl text-sm font-semibold">Save Rules</button>
        </div>
      </div>
    </div>
  </div>
  ...
  new Chart(document.getElementById('aiPerfChart').getContext('2d'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'ROAS Accuracy', data: [78,81,84,86,88,91,93,94.2], borderColor:'#FF4D00', fill:false, tension:0.4, pointRadius:3, borderWidth:2 },
    ...
    options: { responsive:true, maintainAspectRatio:false,
      ...
      scales:{
        x:{ grid:{color:'rgba(255,255,255,0.04)'}, ticks:{color:'#7A7A7A',font:{size:9}} },
        y:{ grid:{color:'rgba(255,255,255,0.04)'}, ticks:{color:'#7A7A7A',font:{size:9}, callback:v=>v+'%'}, min:65 }
      }
    }
  });
  ...
  function togglePauseAI() {
    ...
    if (aiPaused) {
      ...
      dot.classList.remove('bg-brand-500','animate-pulse');
      dot.classList.add('bg-brand-500');
      sub.textContent = 'AI Engine is paused — no autonomous actions';
      showToast('AI Engine paused — all automation suspended', 'brand');
    } else {
      icon.className = 'fas fa-pause';
      label.textContent = 'Pause AI';
      badge.textContent = 'ACTIVE';
      badge.classList.add('badge-live');
      badge.classList.remove('bg-brand-500/20','text-brand-400');
      ...
      showToast('AI Engine resumed — autonomous optimization active', 'brand');
    }
  }
  ...
  function saveConfig() {
    closeConfigModal();
    showToast('AI Engine configuration saved successfully', 'brand');
  }
