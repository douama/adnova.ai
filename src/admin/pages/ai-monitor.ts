...
export const renderAdminAIMonitor = (c: Context) => {
  ...
  <div class="glass rounded-2xl p-5 mb-6 border border-orange-500/15">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-500 to-slate-600 flex items-center justify-center shadow-xl">
        ...
        <div>
          ...
          <div class="flex gap-4 mt-1 text-xs text-slate-600">
            ...
            <span><i class="fas fa-microchip text-slate-400 mr-1"></i>48/64 GPUs actifs</span>
            <span><i class="fas fa-memory text-brand-400 mr-1"></i>RAM: 78%</span>
          </div>
        </div>
      </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ${infraMetric('CPU Global', '73%', 'fa-microchip', 73, 'slate')}
    ${infraMetric('RAM Utilisée', '78%', 'fa-memory', 78, 'brand')}
    ${infraMetric('GPU Load', '75%', 'fa-bolt', 75, 'orange')}
    ${infraMetric('API Latence', '42ms', 'fa-wifi', 42, 'brand')}
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="glass rounded-2xl p-5">
      ...
      <div style="position:relative;height:240px"><canvas id="aiDecisionsChart"></canvas></div>
    </div>
    <div class="glass rounded-2xl p-5">
      <h3 class="font-bold text-white mb-4">Répartition Actions IA</h3>
      <div style="position:relative;height:240px"><canvas id="aiActionsChart"></canvas></div>
    </div>
  </div>
  ...
  <div class="glass rounded-2xl p-5 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ${aiModel('AdNova-Predictor-v2', 'Performance prediction', '94.2%', 'Actif', '2.1M', 'brand', '12h')}
      ${aiModel('Creative-Gen-v3', 'Image/Video generation', '—', 'Actif', '847K/j', 'brand', '6h')}
      ${aiModel('Budget-Opt-v2', 'Budget optimization', '91.3%', 'Actif', '2.4M', 'slate', '3h')}
      ${aiModel('Copy-Engine-v1', 'Ad copy generation', '88.7%', 'Actif', '380K', 'brand', '24h')}
      ${aiModel('Audience-ML-v2', 'Lookalike building', '84.2%', 'Actif', '124K', 'slate', '48h')}
      ${aiModel('Creative-Scorer-v1', 'CTR prediction', '87.6%', 'Actif', '5.8M', 'brand', '6h')}
    </div>
  </div>
  ...
  <div class="glass rounded-2xl p-5">
    ...
    <div class="space-y-2">
      ${incident('Résolu', 'brand', 'Pic CPU 92% — AI predictor', 'Auto-scaled: +4 GPUs ajoutés', '29 mar 2026 14:32', '8 min')}
      ${incident('Résolu', 'brand', 'Latence API +120ms', 'Cache invalidé — Redis flushed', '28 mar 2026 09:18', '3 min')}
      ${incident('Résolu', 'brand', 'Erreur rate +0.3%', 'TikTok API timeout — retry logic', '27 mar 2026 22:45', '15 min')}
    </div>
  </div>
  ...
  <div id="maintenance-modal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="glass rounded-2xl w-full max-w-md border border-orange-500/30">
      ...
      <div class="p-5 space-y-4">
        <div class="p-3 rounded-xl bg-brand-500/10 border border-brand-500/20 text-xs text-brand-300 flex items-start gap-2">
        ...
        <div class="flex gap-2">
          ...
          <button onclick="activateMaintenance()" class="flex-1 bg-gradient-to-r from-brand-600 to-orange-600 hover:opacity-90 text-white py-2.5 rounded-xl text-sm font-bold transition-all">
  ...
  <div id="deploy-modal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="glass rounded-2xl w-full max-w-md border border-orange-500/30">
      <div class="p-5 border-b border-white/10 flex items-center justify-between">
        <h2 class="font-bold text-white flex items-center gap-2"><i class="fas fa-rocket text-brand-400"></i> Déployer un modèle IA</h2>
      ...
      <div class="p-5 space-y-4">
        ...
        <div class="flex gap-2">
          ...
          <button onclick="deployModel()" class="flex-1 bg-gradient-to-r from-brand-600 to-brand-600 hover:opacity-90 text-white py-2.5 rounded-xl text-sm font-bold transition-all">
  ...
  function toggleGlobalPause() {
    ...
    if (aiPaused) {
      ...
      btn.className = 'bg-brand-500/20 border border-brand-500/30 text-brand-400 text-xs px-4 py-2 rounded-xl transition-all';
      if (badge) { badge.className = 'badge-paused text-xs px-2 py-0.5 rounded-full font-bold'; badge.textContent = 'EN PAUSE'; }
      showAdminToast('⏸ AI Engine mis en pause globale — toutes décisions suspendues', 'brand');
    } else {
      ...
      showAdminToast('▶️ AI Engine réactivé — décisions reprises', 'brand');
    }
  }
  ...
  function activateMaintenance() {
    ...
    showAdminToast('🔧 Mode maintenance activé (' + label + ') — ' + reason, 'brand');
    if (duration !== '0') {
      setTimeout(() => showAdminToast('✓ Maintenance terminée — AI Engine opérationnel', 'brand'), parseInt(duration) * 1000);
    }
  }
  ...
  function deployModel() {
    closeDeployModal();
    showAdminToast('🚀 Déploiement lancé — monitoring actif pour les 30 prochaines minutes', 'brand');
  }
  ...
  window.addEventListener('load', function() {
    ...
    new Chart(document.getElementById('aiDecisionsChart').getContext('2d'), {
      ...
      options: { responsive:true, maintainAspectRatio:false,
        ...
        scales:{
          x:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#7A7A7A',font:{size:9}}},
          y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#7A7A7A',font:{size:9},callback:function(v){return (v/1000).toFixed(0)+'K'}}}
        }
      }
    });
    new Chart(document.getElementById('aiActionsChart').getContext('2d'), {
      ...
      data: {
        ...
        datasets:[{ data:[35,24,18,12,8,3],
          backgroundColor:['rgba(255,77,0,0.85)','rgba(122,122,122,0.8)','rgba(255,77,0,0.85)','rgba(168,168,168,0.8)','rgba(255,107,43,0.85)','rgba(152,152,184,0.5)'],
          borderWidth:0, hoverOffset:4 }]
      },
      options:{ responsive:true, maintainAspectRatio:false, cutout:'65%', plugins:{legend:{position:'right',labels:{color:'#7A7A7A',font:{size:10}}}} }
    });
  });
  </script>
  `
...
function infraMetric(label: string, value: string, icon: string, pct: number, color: string): string {
  const barColor = pct > 85 ? 'slate' : pct > 70 ? 'brand' : color
...
function incident(status: string, color: string, title: string, resolution: string, time: string, duration: string): string {
  return `<div class="flex items-start gap-3 p-3 glass rounded-xl">
    <span class="badge-${color === 'brand' ? 'active' : 'trial'} text-xs px-2 py-1 rounded-full flex-shrink-0 mt-0.5">${status}</span>
